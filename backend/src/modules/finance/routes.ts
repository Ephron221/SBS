...
export function registerFinanceRoutes(app: any, _store: any) {
  const router = Router()
  ...
  router.get('/expenses', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (_req: AuthRequest, res: Response) => {
    const expenses = await db.expense.findMany({ orderBy: { date: 'desc' }, include: { createdBy: { select: { name: true } } } })
    res.json({ success: true, data: expenses })
  })

  router.post('/expenses', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {

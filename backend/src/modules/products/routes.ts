...
async function notifyStockLeaders(title: string, message: string) {
  await db.notification.createMany({
    data: [UserRole.SUPER_ADMIN, UserRole.MANAGER].map(role => ({ title, message, type: 'stock', role })),
  })
}

export function registerProductRoutes(app: any, _store: any) {
  const router = Router()
  ...
  router.post('/', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
  ...
  router.put('/:id', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
  ...
  router.get('/:id/history', authenticate(_store), async (req: Request, res: Response) => {
    const id = String(req.params.id)
    try {
  ...
  stockRouter.post('/adjust', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
    const { productId, delta, reason } = req.body as { productId: string; delta: number; reason: string }
    const existing = await db.product.findUnique({ where: { id: productId } })
    if (!existing) return res.status(404).json({ success: false, message: 'Product not found.' })
    if (existing.currentQuantity + delta < 0) return res.status(400).json({ success: false, message: 'Stock cannot go negative.' })
  ...
  categoryRouter.post('/', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
    const { name } = req.body
  ...
  unitRouter.get('/', async (_req: Request, res: Response) => {
    try {
  ...
  unitRouter.post('/', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
    const { name } = req.body
    if (!name) return res.status(400).json({ success: false, message: 'Unit name is required.' })
    try {
      if (!(db as any).unit) return res.status(400).json({ success: false, message: 'Units table not ready. Run migrations.' })

...
export function registerCustomerRoutes(app: any, _store: any) {
  const router = Router()
  ...
  router.post('/', authenticate(_store), async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (err.code === 'P2002') {
        const target = (err.meta?.target as string[])?.join(', ') || 'phone or email'
  ...
  router.get('/:id', authenticate(_store), async (req: Request, res: Response) => {

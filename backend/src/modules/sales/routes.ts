...
export function registerSalesRoutes(app: any, _store: any) {
  ...
  router.post('/', authenticate(_store), async (req: AuthRequest, res: Response) => {
    ...
    if (!items?.length) return res.status(400).json({ success: false, message: 'Sale items are required.' })

    let subtotal = 0
    const enriched: Array<{ productId: string; quantity: number; unitPrice: number; lineTotal: number }> = []
  ...
  router.delete('/:id', authenticate(_store), restrictTo('MANAGER', 'SUPER_ADMIN'), async (req: AuthRequest, res: Response) => {
